...
function initSocket(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('create_game', (data) => {
      ...
      console.log(`[Game] ${playerName} criou a partida ${gameId}`);
    });
    ...
    socket.on('join_game', (data) => {
      ...
      const result = gameManager.joinGame(
        { socketId: socket.id, playerName, gameId },
        {
          ...
          onGameOver: ({ winner, scores }) => {
            ...
            console.log(`[Game] Partida ${gameId} encerrada. Vencedor: ${winner}`);
          },
          onLog: (msg) => {
            io.to(gameId).emit('game_log', msg);
          },
        }
      );
      ...
      console.log(`[Game] ${playerName} entrou na partida ${gameId}`);
    });
